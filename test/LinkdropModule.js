...
describe('Linkdrop Module Tests', () => {
  ...
  it('should topup gnosis safe with tokens', async () => {
    const tokenBalanceBefore = await tokenInstance.balanceOf(gnosisSafe.address)
    let amount = 100000
    await tokenInstance.transfer(gnosisSafe.address, amount)
    const tokenBalanceAfter = await tokenInstance.balanceOf(gnosisSafe.address)
    expect(tokenBalanceAfter).to.eq(tokenBalanceBefore + amount)
  })

  it('should successfully claim ethers and tokens', async () => {
    const receiverWeiBalanceBefore = await provider.getBalance(receiverAddress)
    ...
    expect(receiverWeiBalanceAfter).to.eq(receiverWeiBalanceBefore + weiAmount)
